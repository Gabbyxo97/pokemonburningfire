...
static u8 PlayerHasEnoughPokemonToTrade_HandleMewDeoxys(u8 *flags, u8 partyCount, u8 cursorPos)
{
    ...
    if ((species == SPECIES_DEOXYS  || species == SPECIES_DEOXYS_ATTACK || species == SPECIES_DEOXYS_DEFENSE || species == SPECIES_DEOXYS_SPEED || species == SPECIES_MEW) && !GetMonData(&gEnemyParty[sTradeMenuResourcesPtr->otherPlayerCursorPosition % 6], MON_DATA_EVENT_LEGAL))
        return 2;
    if (count != 0)
        count = 1;
    return count;
...
static u32 TestWhetherSelectedMonCanBeTraded(struct Pokemon * party, int partyCount, int cursorPos)
{
    ...
    if (species[cursorPos] == SPECIES_DEOXYS || species[cursorPos] == SPECIES_DEOXYS_ATTACK || species[cursorPos] == SPECIES_DEOXYS_DEFENSE || species[cursorPos] == SPECIES_DEOXYS_SPEED || species[cursorPos] == SPECIES_MEW)
    {
...
{
    if (species == SPECIES_DEOXYS || species == SPECIES_DEOXYS_ATTACK || species == SPECIES_DEOXYS_DEFENSE || species == SPECIES_DEOXYS_SPEED || species == SPECIES_MEW)
    {
        if (!isEventLegal)
            return TRUE;
    }
